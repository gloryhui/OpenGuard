...
- 그룹과 규칙 추가, 드래그 정렬과 그룹 간 이동을 지원합니다. 그룹 행에는 편집·위로·아래로·삭제 아이콘이 있습니다. 삭제 시 그룹만 삭제(규칙과 앱 연결을 루트에 유지), 그룹과 규칙 삭제, 취소 중 선택합니다.
...
2. 상단 버튼에서 그룹이나 규칙을 추가하고 드래그하여 정렬하거나 다른 그룹 및 루트로 이동합니다.
   ...
   선택한 그룹 안에 규칙 행을 추가하고 확장자와 표시 이름을 직접 편집합니다. 평소에는 연필과 휴지통, 편집 중에는 체크와 × 아이콘이 표시됩니다. 우클릭으로도 편집하거나 삭제할 수 있습니다. 취소하면 변경만 버리고 행은 유지하며, 빈 규칙은 보호에 참여하지 않습니다.
