...
- **完整的分组管理**：可新增分组和规则、拖动排序、跨组移动；分组行提供编辑、上移、下移和删除图标。点击分组删除图标时可选择仅删除分组（规则移到根目录）、删除分组和规则，或取消。
- **键盘与多选**：列表支持 Shift 点击连续多选、⌘ 点击增减选择、⌘A 全选。单选或多选后按 Delete 会先确认；选中分组时，其全部规则（含折叠隐藏的规则）也会删除。输入框内的快捷键仍用于编辑文字。
...
2. 选中分组后点击“添加规则”，会直接在组内新增可编辑明细；填写后缀名和显示名称，点击 ✓ 确认。正常行右侧为铅笔（编辑）和垃圾桶（删除），编辑时为 ✓（确认）和 ×（取消）；右键也可编辑或删除。取消只放弃修改，保留条目；未填写的规则不会参与保护。拖动行可排序或跨组移动。
3. 点击分组右侧的 `…` 统一指定应用，或点击单个后缀右侧的 `…` 设置覆盖规则。右键分组可以重命名。
4. 移动完成后，可启用“登录时启动 OpenGuard”，并保持自动恢复开启。

关闭设置窗口后，菜单栏图标仍会继续工作；主界面激活时按 ⌘Q 可退出整个程序并停止守护。应用作为菜单栏工具运行，不显示 Dock 图标。选择“仅删除分组”会保留规则及应用绑定并移到根目录；“删除分组和规则”会移除该组及组内规则。“初始化分组”位于顶部“设置”菜单中，执行前会明确警告并二次确认。
